...
setwd("C:/Users/Axel/Desktop/English_version")
...
ui <- navbarPage("Artificial land use in European cities in 2006", 
            ...
            tabPanel("Interactive map", 
                     ...
                     h5("Contributors : Axel PECHERIC, Marion LE TEXIER, Rémi LEMOY"), br()),   
                 
        navbarMenu("Methodology",
            tabPanel("Urban scaling laws",
                     fluidRow(column(11, h4("Scaling can be defined as the ability to transform a set of objects from one spatial scale 
                                            to another without changing the structure (Batty, 2015). Without scaling, comparison between 
                                            cities of different sizes is meaningless because the urbanization process is much more 
                                            advanced in larger cities."))), 
                     fluidRow(column(11, h4("As Lemoy and Caruso (2018) have shown, through scaling, the organization of  successions 
                     ...
                     h4("Variation of the exponent to obtain the optimal scaling factor. - 
                    ...
                    fluidRow(column(2, uiOutput("villes1")),
                             ...
                             column(3, chooseSliderSkin(skin = "HTML5", color = "MediumBlue"),
                                    sliderInput("distance", label = "Choice of distance in km",
                                                min = 0.5, max = 70, step = 0.5, value = 30)),
...
server <- function(input, output) {
  ...
  output$InfoBox1 <- renderUI({
    if (input$Info1 %% 2){
      helpText(tags$div(HTML("</br> <FONT size='3pt'>
                             <b> Calculating the scaling factor: </b> </FONT>")), 
  ...
  output$DistanceDisque <- renderUI({
    tags$div(HTML("<FONT size='3pt'>
           ...
           as the reference city for the scaling factors.</br></br>    
           ...
           The radius of each disc is calculated by dividing the selected distance by the scaling 
           factor of each city.
           </FONT>"))
  })
  ...
  output$InfosRescaling2 <- renderUI({
    tags$div(HTML("<FONT size='4pt'> <b> Scaling factors </b> </FONT> </br>", 
  ...
  output$NoteLecture2 <- renderUI({
    tags$div(HTML("<FONT size='3pt'>
                  <b> Note for the reader: </b> </br>
                  This synthesis shows that the scaling by the square root of the population of each city is quite
  ...
  output$NoteLecture3 <- renderUI({
    tags$div(HTML("<FONT size='3pt'>
                  <b> Note for the reader: </b> </br>
                  Using urban scaling laws, Madrid obtains a scaling factor of 1.421. 
  ...
  output$NoteLecture4 <- renderUI({
    tags$div(HTML("<FONT size='3pt'>
                  <b> Note for the reader: </b> </br>
                  <b> The black dotted lines show the European average of artificial land use shares. </b> </br> </br>
  ...
  output$NoteLecture5 <- renderUI({
    tags$div(HTML("<FONT size='3pt'>
                  ...
                  The black dotted lines show the European average of artificial land use shares.
                  </FONT>"))
  })
